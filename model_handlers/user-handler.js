...
const getSort = async(requestParam) => {
    return new Promise(async(resolve, reject) => {
        try {
            ...
            let count = await query.joinWithAnd(dbConstants.dbSchema.users, joinArr);
...
const create = async(requestParam) => {
    return new Promise(async(resolve, reject) => {
        try {
            ...
            if(response){
                reject(errors(labels.LBL_EMAIL_ALREADY_EXISTS[config.default_language], responseCodes.ResourceNotFound));
                return;
            }
