...
const access_rights = require('./../models/access-right');
...
const create = async(requestParam) => {
    return new Promise(async(resolve, reject) => {
        try {
            ...
            let res = await query.insertSingle(dbConstants.dbSchema.roles, requestParam);
            accessRightInsert(res.role_id);
            resolve({});
            return;
        } catch (error) {
            console.log(error)
...
const update = async(requestParam, req) => {
    return new Promise(async(resolve, reject) => {
        try {
            ...
            accessRightInsert(requestParam.role_id);
            resolve({});
            return;
        } catch (error) {
            reject(error)
...
const accessRightInsert = async(role_id) => {
    return new Promise(async(resolve, reject) => {
        try {
            let response = await query.selectWithAnd(dbConstants.dbSchema.modules, {}, { _id: 0}, { order_no: 1 });
            let obj = {}
            ...
            let exists = await query.selectWithAndOne(dbConstants.dbSchema.access_rights, {role_id}, { _id: 0, role_id:1}, {});
            if(exists){
                await query.updateSingle(dbConstants.dbSchema.access_rights, insertObj, {role_id});
            }
            else{
                await query.insertSingle(dbConstants.dbSchema.access_rights, insertObj);
            }
            return false;
        } catch (error) {
            reject(error)
